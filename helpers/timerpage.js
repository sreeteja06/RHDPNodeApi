...
const timerpageApi = (packets, date_time) => {
  ...
  let IT1 = new Date(date_time[0].getTime() + laststageTime.getTime());
  laststageTime = new Date(LS3 * 1000);
  let IT2 = new Date(date_time[1].getTime() + laststageTime.getTime());
  console.log("IT1: " + IT1);
  console.log("IT2: " + IT2);
  ...
  if (timeLeft.reduce((a, b) => a + b, 0) == 0) {
    ...
    details_dict["trafficPercent"] = Math.round(
    ...
    console.log(
      "time Taken overall(in seconds):" +
        (details_dict["systime"] - start) / 1000
    );
    return details_dict;
  }
};
