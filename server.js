...
app.post("/newJunctionPoint", authenticate, (req, res) => {
	//to update a new location in the database
...
app.get("/getLocations", authenticate, (req, res) => {
	//to get the locations based on who is logged on
	JunctionPoint.find({
...
app.post("/giveLocationAccess", authenticate, (req, res) => {
	if (req.user._id.toString() === admin_id.toString()) {
		//only admin has the access to give the access
		JunctionPoint.updateOne(
			{ _id: req.body.locationID.toString() },
			{
				$push: {
					_accessedByUsers: { _id: ObjectID(req.body.addUserid.toString()) }
				}
			}
		)
			.then(response => {
...
app.delete("/removeJunctionPoint", authenticate, (req, res) => {
	//removes the junctionpoint given the locationid
	if (req.user._id.toString() === admin_id.toString()) {
		// if only admin can remove the location
		JunctionPoint.remove({ _id: ObjectID(req.query.locationID.toString()) })
			.then(result => {
				res.send(result);
			})
			.catch(e => {
